...
function wpthumb_wm_watermark_preview_image_ajax() {
	...
	<a target="_blank" href="<?php echo wpthumb( $image, array( 'width' => 1000, 'height' => 0, 'crop' => false, 'resize'	=> true, 'watermark_options' => $large_watermark, 'cache' => false ) ); ?>">View Large</a>
...
function wpthumb_media_form_watermark( $fields, $post ) {
	...
	<div rel="<?php echo $post->ID; ?>" class="wm-watermark-options">
		...
		<div class="watermark_pos" style="display: <?php echo wpthumb_wm_image_has_watermark( $post->ID ) ? 'block' : 'none'; ?>;">
			...
			<span class="wpthumb_wrap">
				...
				<label>Padding</label>
					...
					<input class="wm_watermark_padding" type="text" value="<?php echo wpthumb_wm_padding( $post->ID ); ?>" style="width:30px" name="attachments[<?php echo $post->ID; ?>][wpthumb_wm_watermark_padding]">px
				</span>
			...
			<span class="wpthumb_wrap">
				...
				<select name="attachments[<?php echo $post->ID; ?>][wm_watermark_mask]" class="wm_watermark_mask">
					<?php  echo $watermark_masks_options_html; ?>
					</select>
				</span>

				<span class="wpthumb_wrap">
					<a class="button preview-watermark" href="#">Preview</a> 
					<?php echo $calling_post_id ? '<a class="button-primary save-watermark" href="#">Save</a>' : ''; ?>
			</span>
			
			<br />
			
			<span class="wm-watermark-preview"></span>
	...
	<script type="text/javascript">
		...
		jQuery( document ).ready( function() { 
			...
			function WMCreatePreview( optionsPane ) {
				...
				// Show loading
				jQuery( optionsPane ).find( '.wm-watermark-preview' ).html( '<span class="wm-loading">Generating Preview&hellip;</span>' );

				if ( typeof( WMCreatePreviewXHR) != 'undefined' )
					WMCreatePreviewXHR.abort();

				WMCreatePreviewXHR = jQuery.get( ajaxurl, { action: 'wpthumb_wm_watermark_preview_image', 'position' : position, 'padding' : padding, 'image_id' : jQuery( optionsPane ).attr( 'rel' ), 'mask' : mask },
			...
			function WMSaveWatermark( optionsPane, elem ) {
				...
				WMSaveWatermarkXHR = jQuery.post( ajaxurl, {
					...
					function( data ) {
						...
						<?php if ( $calling_post_id && function_exists( 'get_post_thumbnail_id' ) && get_post_thumbnail_id( $calling_post_id ) ) : ?>
		...
		<style>
			.watermark_pos input { margin: 5px; } .wpthumb_wrap { display: inline-block; padding: 0 5px; }
			...
			.wm-loading { line-height: 16px; text-align: center; width: 120px; background: url(<?php site_url( '/wp-admin/images/loading.gif' ); ?>) no-repeat; padding-left: 20px; padding-top: 1px; padding-bottom: 2px; font-size: 11px; color: #999; }
		</style>
